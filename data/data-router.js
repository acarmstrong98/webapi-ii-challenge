...
router.get("/", async (req, res) => {
  try {
    ...
    console.log(error);
    res.status(500).json({error: "This posts information could not be retrieved."});
  }
});
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    const post = await Posts.update(req.params.id, { title, contents });
      ...
      res.status(400).json({error: "Please provide title and content for the post." });
    }
    ...
    console.log(error);
    res.status(500).json({
      message: "This posts information could not be modified.",
    });
  }
});
